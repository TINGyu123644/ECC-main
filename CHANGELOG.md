...
- Wrapper-level `loop-orchestrator/` (autonomous 7-step driver) and its wrapper-side SPEC.md, USAGE.md, AGENTS.md, BOOTSTRAP.md, RISK.md, ROUTING.md, DELIVERY_SEG2.md, agent/role files, scripts, hooks, and templates have been retired. The `ECC-main/` submodule remains the single source of truth; users wanting autonomous-loop behavior should adopt the in-submodule `loop-orchestrator/SPEC.md` design directly. The wrapper root no longer carries loop-orchestrator scripts, state files, or wrappers (`install.js`, `autopilot.js`, `state.py`, `verify.sh`, etc.).
...
- ECC orchestration mechanism reframed in `ECC-技能编排机制.md` v2.0 (2026-07-30) as **4 类文件 + 1 条数据流**: 1 shared engine (`skills/orch-pipeline/SKILL.md`, 122 lines), 5 thin-wrapper playbook skills (`skills/orch-{add-feature, change-feature, fix-defect, refine-code, build-mvp}/SKILL.md`, ~45 lines each), 6 entry commands (`commands/orch-{add-feature, change-feature, fix-defect, refine-code, build-mvp, review}.md`, ~30 lines each), and ⭐ 1 real hard-code path (`workflows/orch-review.workflow.js`, 296 lines, the only fail-closed gate enforcing Phase 5 Review with multi-dimension parallel review, dedup, and adversarial verification at `REFUTE_MIN_CONFIDENCE = 0.8`). Size classifier upgraded to a 4-tier table (trivial/small/standard/large) mapping task signals to phase subsets (3–6 phases). Honest disclosure added: only Phase 5 Review has fail-closed enforcement; Phases 0–4 and 6 are Markdown-protocol-only and rely on AI self-discipline for Gate 1 (post-Plan) and Gate 2 (pre-Commit). v1.0 claims now explicitly retired: JSON Schema contracts for phase I/O, fail-closed stage gates, and `size-classify.js` runtime impl — none of these exist as code. Also clarified that `scripts/release-approval-gate.js` is **not** an orchestration gate (name collision with release-flow gate).
- ECC progressive-loading skill selection mechanism reframed in `ECC-技能选择机制-渐进式加载.md` v1.2 (2026-07-30): trimmed from 7 methods to **5 methods** by retiring two that did not perform real loading convergence — `agent-sort` 二级分桶 (filtering by bucket, not loading) and `plan-orchestrate` Scope 下钻 (output filtering, not loading). The retained 5 split into two tiers: 段一 (code-level, ⭐ globally enforced) — Method 1 `scripts/catalog.js` 三级下钻 (profile → component → module) and Method 2 `scripts/consult.js` Top-N 限流 (`DEFAULT_LIMIT = 5`, `.slice(0, limit)` with fuzzy + ML-context + preferred-bonus scoring); 段二 (convention-level, ◐ scoped to triggered skills) — Method 3 multi-round narrowing (`skills/team-builder/SKILL.md` Step 2-4: domain → agent name → ≤5 hard cap), Method 5 Phase Mask (`skills/orch-pipeline/SKILL.md` Step 0: 4-tier → phase subset, applies to the 5 orch-* playbooks only), Method 7 Description frontmatter-first reading (every SKILL.md, enforced by `js-yaml` not expanding body and reused by Method 2's scoring corpus).
